...
class WTFApp:
    ...
    def _create_widgets(self) -> None:
        ...
        ttk.Button(
            model_frame, text="Download Models", command=self._download_models
        ...
        thumb_frame = ttk.LabelFrame(output_frame, text="Preview", padding="5")
        thumb_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))

        self.thumbnail_label = ttk.Label(thumb_frame, text="No preview")
    ...
    def _update_stats(self) -> None:
        """Update entry and error counts from diary."""
        path = Path(self.diary_path.get())
        self.entry_count.set(diary.get_entry_count(path))
        self.error_count.set(diary.get_error_count(path))
    ...
    def _start_recording(self) -> None:
        ...
        if not config.is_model_downloaded(self.selected_model.get()):
            messagebox.showerror(
                "Model Not Found",
                f"Model '{self.selected_model.get()}' is not downloaded.\n\n"
                "Please click 'Download Models' first.",
            )
            return
        ...
        while not self.stop_event.is_set():
            ...
            for idx, screenshot in enumerate(screenshots):
                ...
                try:
                    ...
                    self.root.after(0, lambda t=thumb: self._update_thumbnail(t))
                except Exception:
                ...
                try:
                    ...
                    self.root.after(
                        0, lambda r=result, t=timestamp: self._set_last_text(r, t)
                    )
